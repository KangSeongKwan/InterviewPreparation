...
### 6-4. OSI 7계층
- 개방형 시스템 상호연결 모델의 표준
- 실제 인터넷에서 사용하는 TCP/IP는 OSI 참조 모델을 기반으로 상업/실무적으로 이용될 수 있도록 단순화한 것
- 작동원리는 다음과 같다.  
![image](https://user-images.githubusercontent.com/99636945/210474428-94a94297-99ae-4c9f-bdb1-2d832aec1569.png)
![images_cgotjh_post_52907c8c-c149-4943-ad21-3996f44f912f_995EFF355B74179035](https://user-images.githubusercontent.com/99636945/210475462-8654a8ff-84da-46bc-90bb-a3bbb395791a.jpg)
#### 6-4.1 물리 계층
- 7계층 중 최하위 계층으로 전기,기계, 기능적인 특성을 이용해 데이터를 전송한다.
- 알고리즘 및 오류제어 기능은 없음
- 장비로는 케이블, 리피터, 허브가 있음
#### 6-4.2 데이터 링크 계층
- 물리적인 연결을 통하여 인접한 두 장치간의 신뢰성 있는 정보전송을 담당
- 오류제어 및 재전송 기능이 존재
- MAC주소를 활용하여 전송하며 데이터 단위는 frame이다.
- 장비로는 브릿지, 스위치가 있음
#### 6-4.3 네트워크 계층
- 중계 노드를 통하여 전송하는 경우 어떻게 중계하는지 결정
- 목적지까지 가장 안전하고 빠르게 데이터를 보내는 기능을 가지고 있음(최적의 경로설정 가능)
- 컴퓨터에게 데이터를 전송할 주소를 갖고있어서 통신가능(IP주소가 네트워크 계층 헤더에 속함)
- 데이터 단위는 패킷이고, 장비로는 라우터, L3스위치가 있음
#### 6-4.4 전송계층
- 종단 간 신뢰성있고 정확한 데이터 전송을 담당
- 송신자와 수신자 간 신뢰성있고 효율적인 데이터를 전송하기 위해 오류검출 및 복구, 흐름제어와 중복검사 등을 수행
- Port 번호를 사용함.(프로토콜로는 TCP와 UDP가 있음)
- 데이터 단위는 세그먼트이다.
#### 6-4.5 세션게층
...
- 세가지의 기능을 가지고 있음  
![image](https://user-images.githubusercontent.com/99636945/210478437-4c22a06d-4558-4c18-99f7-1f1ba36e621d.png)
#### 6-4.7 응용계층
- 사용자와 가장 밀접한 계층으로 인터페이스 역할
- 응용 프로세스 간의 정보 교환을 담당
- 예시로는 전자메일, 인터넷, 동영상 플레이어 등이 있다.

### 6-5. 프로토콜
- 컴퓨터 또는 원거리 통신장비 사이에서 메시지를 주고받는 양식과 규칙 체계
- 기본 요소로는 구문, 의미, 시간이 있다.  
![image](https://user-images.githubusercontent.com/99636945/210479685-1d13669b-72ba-4acb-a1d2-a53d9e82da5a.png)
- 프로토콜의 종류는 다음 표와 같이 정리된다.  
![image](https://user-images.githubusercontent.com/99636945/210479838-4d743d27-aaeb-496d-8377-ab7ba16bb535.png)
- 프로토콜의 기능에는 단편화와 재합성, 캡슐화, 연결제어, 흐름제어 등등 다양한 기능이 있다.
#### 6-5.1 단편화와 재합성
- 단편화 : 송신측에서 긴 데이터 블록을 손쉽게 전송할 수 있도록 크기가 똑같은 작은 블록으로 나누어 전송
- 재합성 : 수신측에서 쪼개진 작은 데이터 블록을 재합성하여 원래의 메시지로 복원하는 기능
#### 6-5.2 캡슐화
- 각 프로토콜에 적합한 데이터 블록을 만들려고 데이터에 정보를 추가하는 것
- 플래그, 주소, 제어정보, 오류검출 부호 등을 부착하는 기능
#### 6-5.3 연결 제어
- 비연결 데이터 전송과 연결 위주 데이터 전송을 위한 통신로를 개설,유지,종결하는 기능
#### 6-5.4 흐름 제어
- 데이터 양이나 통신 속도 등이 수신측의 처리 능력을 초과하지 않도록 조정하는 기능
#### 6-5.5 순서 결정
- 연결 위주의 데이터를 전송할 때 송신츠깅 보내는 데이터 단위 순서대로 수신측에 전달하는 기능
#### 6-5.6 주소 설정
...
- 비동기 방식은 응담 상태와 상관 없이 다음 동작을 수행할 수 있다.
...
- 크리티컬 섹션 기반 동기화도 내부적으로는 인터락 함수 기반으로 구현되어 있음
